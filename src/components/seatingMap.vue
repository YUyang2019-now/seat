    ...
    <div class="map-wrap">
        ...
        <div class="map-right-wrap" id="mapWrap">
            ...
            <div class="detail-right-wrap">
                ...
                <canvas id="seatMap" @mousedown="mousedown" @mouseup="mouseup">你的浏览器不支持canvas,请升级你的浏览器</canvas>
            </div>
        ...
        <el-form :model="areaForm">
            ...
            <el-form-item label="背景色" :label-width="formLabelWidth">
                <el-select v-model="queryColor">
                    <el-option v-for="item in colorList" :key="item.value" :value="item.label" :label="item.label" class="set-flex">
                        <p class="selectColor" :style="{backgroundColor: item.label}"></p><span>{{item.label}}</span>
                    </el-option>
                </el-select>
            </el-form-item>
        </el-form>
...
<script>
import seat2 from '../../static/seatMap/seat2.png'
import toilet from '../../static/seatMap/toilet.png'
export default {
    name: 'seatingMap',
    data() {
        return {
